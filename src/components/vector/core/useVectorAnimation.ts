...
const applyPulseToVector = (
  ...
  const {
    ...
    distanceImpactFactor = 0.3, // Cómo afecta la distancia al centro (0 a 1)
  ...
  let pulseProgress = Math.min(1, Math.max(0, timeSincePulseStart / pulseDuration));
  ...
  let easedPulseProgress = easingFunction(pulseProgress);
    ...
    const animate = (currentTime: number) => {
      ...
      setAnimatedVectors(prevItems => {
        const newItems = prevItems.map(item => {
          ...
          let targetLengthFactor = 1.0;
          let targetIntensityFactor = 1.0; // Para efectos de intensidad (ej. color, opacidad)
          let targetWidthFactor = 1.0;

          // Lógica de animación existente (switch)
          switch (currentSettings.animationType) {
            case 'smoothWaves':
              targetAngle = calculateTargetAngle_SmoothWaves(newItem, currentTime, currentSettings.animationProps, timeScale);
              break;
            case 'mouseInteraction':
              if (currentMousePos) {
                targetAngle = calculateTargetAngle_MouseInteraction(newItem, currentMousePos, currentSettings.animationProps);
              }
              break;
            case 'flocking':
              // targetAngle = calculateTargetAngle_Flocking(newItem, prevItems, currentDimensions, currentSettings.animationProps);
              break;
            case 'perlinNoise':
              // targetAngle = calculateTargetAngle_PerlinNoise(newItem, currentTime, currentSettings.animationProps);
              break;
            case 'audioReactive':
              // targetAngle, targetLengthFactor, etc. se calcularían basados en datos de audio
              break;
