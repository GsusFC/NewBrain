...
const GridControlSelector: FC<GridControlSelectorProps> = ({
  ...
  const notifyChanges = useCallback((mode: Mode, force = false) => {
    ...
    if (mode === 'manual') {
      const isManualRatioFixed = 
        ...
        !(finalAspectRatio === 'custom' && (!finalCustomRatio || finalCustomRatio.width <= 0 || finalCustomRatio.height <= 0));
      ...
      if (isManualRatioFixed) {
        ...
        // Si manualRows > 0 y manualCols > 0, se usan ambos, no se modifica nada aquí.
      }
    }
  ...
  useEffect(() => {
    if (!isInitialMount.current && state.lastUpdatedMode) {
      ...
      // Considera resetear lastUpdatedMode aquí si causa notificaciones no deseadas en otros flujos
      // Ejemplo: dispatch({ type: 'CLEAR_LAST_UPDATED_MODE' }); // Necesitaría nueva acción
    }
  }, [state.lastUpdatedMode, state.settings, state.ratios, state.customRatios, notifyChanges]); // Depender de los datos asegura la notificación si cambian.
  ...
  const handleDensityChange = useCallback((density: number) => {
    ...
    if (activeCustomRatio.width <= 0 || activeCustomRatio.height <= 0) {
        // console.warn("Custom ratio no válido para calcular densidad.");
        const newSettings = { ...activeSettings, rows: validatedDensity, cols: activeSettings.cols }; // Mantener cols si AR no es válido
